...
var TILES = [
    null,
    new RectSprite('white', new Rect(0, 0, 16, 16)),
    new RectSprite('yellow', new Rect(0, 0, 16, 16)),
];
var isObstacle = (function (c) { return c == 1; });
var isGoal = (function (c) { return c == 2; });
// makeMaze
function makeMaze(tilemap, tile, ratio) {
    ...
    while (0 < pts.length) {
        var i = rnd(pts.length);
        var p0 = pts[i];
        pts.splice(i, 1);
        var j = rnd(dirs.length);
        var t = dirs[0];
        dirs[0] = dirs[j];
        dirs[j] = t;
        for (var _i = 0, dirs_1 = dirs; _i < dirs_1.length; _i++) {
            ...
            var p2 = p1.add(d);
            if (p2.x < 0 || p2.y < 0 ||
                tilemap.width <= p2.x || tilemap.height <= p2.y)
                continue;
            var hole = (tilemap.get(p2.x, p2.y) != tile);
...
var Player = /** @class */ (function (_super) {
    ...
    function Player(tilemap, pos) {
        var _this = _super.call(this, pos) || this;
        _this.goaled = new Signal(_this);
        _this.tilemap = tilemap;
        var sprite = new RectSprite('#0f0', new Rect(-8, -8, 16, 16));
        _this.sprites = [sprite];
        _this.collider = sprite.getBounds();
        _this.usermove = new Vec2();
        _this.prevmove = _this.usermove;
        return _this;
    }
    Player.prototype.onTick = function () {
        _super.prototype.onTick.call(this);
        if (!this.usermove.isZero()) {
            var v = this.moveIfPossible(this.usermove);
            if (v.isZero()) {
...
var Enemy = /** @class */ (function (_super) {
    ...
    function Enemy(tilemap, target, pos) {
        var _this = this;
        var grid = new GridConfig(tilemap);
        var sprite = new RectSprite('#f80', new Rect(-8, -8, 16, 16));
        var speed = new Vec2(2, 2);
        var allowance = 4;
        _this = _super.call(this, tilemap, isObstacle, grid, speed, sprite.getBounds(), pos, allowance) || this;
        _this.sprites = [sprite];
        _this.collider = sprite.getBounds();
        _this.target = target;
        return _this;
    ...
    Enemy.prototype.onTick = function () {
        _super.prototype.onTick.call(this);
        var start = this.grid.coord2grid(this.pos);
        ...
        if (this.runner === null) {
            var action = this.buildPlan(goal, start, 0, 40);
            if (action !== null) {
...
var Game = /** @class */ (function (_super) {
    ...
    Game.prototype.onStart = function () {
        ...
        var rect = this.tilemap.map2coord(new Vec2(1, 1));
        this.player = new Player(this.tilemap, rect.center());
        this.player.goaled.subscribe(function (e) { _this.changeScene(new GoalScene()); });
        this.add(this.player);
        for (var i = 0; i < 10; i++) {
            var x = 3 + 2 * rnd(int((this.tilemap.width - 5) / 2));
            var y = 3 + 2 * rnd(int((this.tilemap.height - 5) / 2));
            var r = this.tilemap.map2coord(new Vec2(x, y));
            var enemy = new Enemy(this.tilemap, this.player, r.center());
            this.add(enemy);
        }
    };
    Game.prototype.onTick = function () {
        _super.prototype.onTick.call(this);
        var target = this.player.getCollider();
    ...
    Game.prototype.render = function (ctx) {
        ...
        fillRect(ctx, this.screen);
        this.tilemap.renderWindowFromBottomLeft(ctx, this.world.window, function (x, y, c) { return TILES[(c < 0) ? 1 : c]; });
        _super.prototype.render.call(this, ctx);
    };
    return Game;
}(GameScene));
var GoalScene = /** @class */ (function (_super) {
    __extends(GoalScene, _super);
    function GoalScene() {
        return _super.call(this, '<strong>Goal!</strong>') || this;
    }
    GoalScene.prototype.onStart = function () {
        _super.prototype.onStart.call(this);
        APP.lockKeys();
        APP.playSound('goal');
    };
    GoalScene.prototype.change = function () {
